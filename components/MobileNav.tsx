...
const MobileNav: React.FC<MobileNavProps> = ({ setPage, activePage }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-6 py-2 md:hidden z-40 pb-safe">
        ...
        {navItems.map((item) => (
          <button
            ...
            className={`flex flex-col items-center gap-1 p-2 min-w-[60px] transition-colors ${
              activePage === item.id ? 'text-brand' : 'text-gray-400 hover:text-gray-600'
            }`}
          >
            {item.icon}
